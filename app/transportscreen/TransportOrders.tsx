...
const TransporterOrders: React.FC = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const availableResponse = await fetch('http://localhost:8080/api/transporter/eligible-orders');
      const availableData = await availableResponse.json();

      const acceptedResponse = await fetch(`http://localhost:8080/api/transporter/${transporterId}/orders`);
  ...
  const handleAcceptOrder = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/transporter/${selectedOrder.orderId}/accept`,
        {
          method: 'POST',
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              const response = await fetch(
                `http://localhost:8080/api/transporter/${orderId}/reject`,
                {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
