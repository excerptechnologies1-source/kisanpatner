...
import { SafeAreaView } from "react-native-safe-area-context";
import { Labourer, NavigationProps } from "../labourscreen/types";
...
export default function LabourListScreen({
  ...
  // Immediate fetch when village filter changes
  useEffect(() => {
    fetchLabourers(searchText, villageFilter);
  }, [villageFilter]);
  ...
  // Filter labourers based on active tab
  const getFilteredLabourers = (): Labourer[] => {
    switch (activeTab) {
      case "present":
        return labourers.filter(l => l.todayAttendance === "present");
      case "absent":
        return labourers.filter(l => l.todayAttendance === "absent");
      default:
        return labourers;
    }
  };
  ...
  const getTabCounts = () => {
    const presentCount = labourers.filter(l => l.todayAttendance === "present").length;
    const absentCount = labourers.filter(l => l.todayAttendance === "absent").length;
    const totalCount = labourers.length;
    
    return { presentCount, absentCount, totalCount };
  };

  const { presentCount, absentCount, totalCount } = getTabCounts();
  const filteredLabourers = getFilteredLabourers();
  ...
  const renderTabButton = (tab: TabType, label: string, count: number) => {
    ...
    return (
      <TouchableOpacity
        ...
        className={`flex-1 flex-row items-center justify-center py-2 px-2 rounded-lg ${
      ...
      >
        ...
        <Text
          className={`text-center flex-1 text-xs mt-1 ${
  ...
  return (
    <>
      <SafeAreaView className="bg-white flex-1">
        ...
        {/* Filters */}
        <View className="space-y-3 gap-3 px-4">
          {/* Search by Name with Filter Icon */}
          <View className="flex-row items-center gap-2">
            <View className="flex-1 flex-row items-center bg-white-100 rounded-lg px-3 py-2 border border-gray-200">
              <Feather name="search" size={16} color="#6B7280" />
  ...
  {/* Tab Navigation */}
        <View className="px-4 py-3 flex-row gap-2">
          {renderTabButton("all", "All Labour", totalCount)}
          {renderTabButton("present", "Present", presentCount)}
        </View>
        ...
        >
          {loading && !refreshing ? (
            ...
            <View className="mt-20 items-center">
              ...
              <Text className="text-gray-500 text-base mt-4 text-center">
                {activeTab === "present" 
                  ? "No present labourers found"
                  : activeTab === "absent"
                  ? "No absent labourers found"
                  : "No labourers found"}
              </Text>
              {(searchText || villageFilter) && (
                <TouchableOpacity onPress={clearFilters} className="mt-3">
