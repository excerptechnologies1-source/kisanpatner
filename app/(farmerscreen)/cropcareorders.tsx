...
const CropcareOrders: React.FC = () => {
  ...
  const getStatusIcon = (status: string) => {
    ...
    switch (status) {
      case 'delivered': return <CheckCircle size={iconSize} color={color} />;
      case 'shipped': return <Truck size={iconSize} color={color} />;
      case 'cancelled': return <XCircle size={iconSize} color={color} />;
      default: return <Clock size={iconSize} color={color} />;
    }
  };
  ...
  const renderOrderItem = (item: OrderItem, index: number) => (
    <View key={index} className="flex-row items-center gap-4 py-3 border-b border-gray-100">
      <View className="w-16 h-16 rounded-lg bg-gray-100 justify-center items-center overflow-hidden">
        {item.image ? (
          <Image 
            source={{ uri: item.image }} 
          ...
          <View className="w-full h-full justify-center items-center bg-emerald-50">
            <Text className="text-2xl">🌱</Text>
          </View>
        )}
      </View>
      <View className="flex-1">
        <Text className="text-base font-medium text-gray-800 mb-1" numberOfLines={2}>
          {item.seedName}
        </Text>
  ...
  const renderOrderCard = ({ item }: { item: Order }) => (
    ...
    >
      <View className="flex-row justify-between items-start mb-4">
        <View className="flex-1">
          <Text className="text-lg font-medium text-gray-900 mb-2">{item.orderId}</Text>
        ...
        <View 
          ...
          style={{ backgroundColor: `${getStatusColor(item.orderStatus)}15` }}
        >
          {getStatusIcon(item.orderStatus)}
          <Text 
      ...
      <View className="pt-4 border-t border-gray-100">
        <View className="flex-row justify-between items-center mb-3">
          <View className="flex-row items-center gap-2">
            <DollarSign size={16} color="#333" />
            <Text className="text-lg font-medium text-gray-900">Total: ₹{item.total.toFixed(2)}</Text>
          </View>
        </View>
        <View className="flex-row gap-3">
          <TouchableOpacity
            className="flex-1 flex-row items-center justify-center gap-2 bg-emerald-600 py-3 rounded-xl"
            onPress={() => viewOrderDetails(item)}
          >
          ...
          {item.orderStatus === 'delivered' && (
            <TouchableOpacity
              className="px-4 py-3 flex-row items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl"
              onPress={() => reorder(item)}
            >
              <ShoppingCart size={14} color="#2c5f2d" />
              <Text className="text-emerald-700 font-medium">Reorder</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
  ...
  const renderLoginPrompt = () => (
    <View className="flex-1 justify-center items-center px-8">
      <View className="w-32 h-32 rounded-full bg-emerald-50 justify-center items-center mb-6">
        <Text className="text-5xl">🔒</Text>
      </View>
      <Text className="text-2xl font-medium text-gray-800 mb-3">Login Required</Text>
      <Text className="text-base text-gray-600 text-center mb-8">
        Please login to view your orders and track your purchases
      </Text>
      <TouchableOpacity
        className="w-full bg-emerald-600 py-4 rounded-xl"
        onPress={() => setShowLoginModal(true)}
      >
        <Text className="text-white font-medium text-center text-lg">Login Now</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  const renderEmptyState = () => (
    <View className="flex-1 justify-center items-center px-8 py-16">
      <View className="w-32 h-32 rounded-full bg-gray-100 justify-center items-center mb-6">
        <ShoppingBag size={48} color="#9CA3AF" />
      </View>
      <Text className="text-2xl font-medium text-gray-700 mb-3">
      ...
      <TouchableOpacity
        className="w-full bg-emerald-600 py-4 rounded-xl flex-row items-center justify-center gap-3"
        onPress={goBackToShop}
      >
        <ShoppingCart size={20} color="#fff" />
        <Text className="text-white font-medium text-lg">Start Shopping</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  if (loading.auth) {
    return (
      <View className="flex-1 bg-gray-50 justify-center items-center">
        ...
        <Text className="mt-4 text-gray-600 font-medium">Checking authentication...</Text>
      </View>
    );
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      {/* Header */}
        ...
        <View className="flex-row pb-5 px-4 py-6 border-b border-gray-200">
          <TouchableOpacity 
            className="w-10 h-10"
            onPress={goBackToShop}
          >
          ...
          <View className="flex-1">
            <Text className="text-xl font-medium text-black">My Orders</Text>
          </View>
          ...
          <>
            ...
            <View className="px-4 pt-6 pb-4">
              ...
              >
                {renderFilterButton('all', 'All Orders', 
                  <Package size={14} color={filter === 'all' ? '#fff' : '#666'} />
                )}
                {renderFilterButton('pending', 'Pending', 
                  <Clock size={14} color={filter === 'pending' ? '#fff' : '#666'} />
                )}
                {renderFilterButton('delivered', 'Delivered', 
                  <CheckCircle size={14} color={filter === 'delivered' ? '#fff' : '#666'} />
                )}
                {renderFilterButton('cancelled', 'Cancelled', 
                  <XCircle size={14} color={filter === 'cancelled' ? '#fff' : '#666'} />
                )}
              </ScrollView>
            </View>
            ...
            <View className="px-4 mb-4">
              <Text className="text-gray-600">
                Showing <Text className="font-medium text-emerald-600">{orders.length}</Text> orders
              </Text>
            </View>
            ...
            {loading.orders ? (
              <View className="flex-1 justify-center items-center">
                <ActivityIndicator size="large" color="#059669" />
                <Text className="mt-4 text-gray-600 font-medium">Loading your orders...</Text>
              </View>
            ) : (
              <FlatList
                data={orders}
      ...
      >
        <View className="flex-1 bg-black/50">
          <View className="absolute bottom-0 w-full bg-white rounded-t-3xl max-h-[90%]">
            {/* Modal Header */}
            <View className="flex-row justify-between items-center p-6 border-b border-gray-100">
              <View className="flex-1">
                <Text className="text-2xl font-medium text-gray-900">Order Details</Text>
                <Text className="text-gray-500 font-medium mt-1">{selectedOrder?.orderId}</Text>
              </View>
              <TouchableOpacity 
              ...
              >
                ...
                <View className="mb-8">
                  <View className="flex-row items-center gap-3 mb-4">
                    ...
                    <Text className="text-xl font-medium text-gray-900">Order Status</Text>
                  </View>
                  <View 
                    className="flex-row items-center gap-3 px-4 py-3 rounded-xl mb-3"
                    style={{ backgroundColor: `${getStatusColor(selectedOrder.orderStatus)}15` }}
                  >
                    {getStatusIcon(selectedOrder.orderStatus)}
                    <Text 
                ...
                <View className="mb-8">
                  <View className="flex-row items-center gap-3 mb-4">
                    ...
                    <Text className="text-xl font-medium text-gray-900">Shipping Address</Text>
                  </View>
                  <View className="bg-gray-50 p-5 rounded-xl border-l-4 border-emerald-500">
                    <Text className="text-lg font-medium text-gray-900 mb-2">
                      {selectedOrder.shippingAddress.name}
                    </Text>
                    <Text className="text-gray-600 mb-2">{selectedOrder.shippingAddress.address}</Text>
                    {selectedOrder.shippingAddress.villageGramaPanchayat && (
                      <Text className="text-gray-600 mb-2">{selectedOrder.shippingAddress.villageGramaPanchayat}</Text>
                    )}
                    <Text className="text-gray-600 mb-2">
                      {selectedOrder.shippingAddress.district}, {selectedOrder.shippingAddress.state} - {selectedOrder.shippingAddress.pincode}
                    </Text>
                    {selectedOrder.shippingAddress.taluk && (
                      <Text className="text-gray-500">Taluk: {selectedOrder.shippingAddress.taluk}</Text>
                    )}
                    {selectedOrder.shippingAddress.post && (
                      <Text className="text-gray-500">Post: {selectedOrder.shippingAddress.post}</Text>
                    )}
                ...
                <View className="mb-8">
                  <View className="flex-row items-center gap-3 mb-4">
                    ...
                    <Text className="text-xl font-medium text-gray-900">Payment Details</Text>
                  </View>
                  <View className="bg-gray-50 p-5 rounded-xl space-y-4">
                    <View className="flex-row justify-between items-center">
                      <Text className="text-gray-600 font-medium">Method:</Text>
                    ...
                    {selectedOrder.payment.razorpayOrderId && (
                      <View>
                        <Text className="text-gray-600 font-medium mb-1">Razorpay Order ID:</Text>
                        <Text className="text-gray-500 text-sm">{selectedOrder.payment.razorpayOrderId}</Text>
                      </View>
                    )}
                  </View>
                </View>
                ...
                <View className="mb-8">
                  <View className="flex-row items-center gap-3 mb-4">
                    ...
                    <Text className="text-xl font-medium text-gray-900">Order Summary</Text>
                  </View>
                  <View className="bg-gray-50 p-5 rounded-xl mb-4">
                    {selectedOrder.items.map((item, index) => (
                  ...
                  <View className="bg-gray-50 p-5 rounded-xl">
                    <View className="flex-row justify-between mb-3">
                      ...
                      <Text className="text-gray-900 font-medium">₹{selectedOrder.subtotal.toFixed(2)}</Text>
                    </View>
                    <View className="flex-row justify-between mb-3">
                      <Text className="text-gray-600">GST (18%):</Text>
                      <Text className="text-gray-900 font-medium">₹{selectedOrder.gst.toFixed(2)}</Text>
                    </View>
                    ...
                    <View className="flex-row justify-between pt-4 border-t border-gray-300">
                      <Text className="text-xl font-medium text-gray-900">Total Amount</Text>
                      <Text className="text-2xl font-medium text-emerald-600">₹{selectedOrder.total.toFixed(2)}</Text>
                    </View>
                  </View>
                </View>
                ...
                <View className="flex-row gap-4 mb-8">
                  {selectedOrder.orderStatus === 'delivered' && (
                    <TouchableOpacity
                      className="flex-1 flex-row items-center justify-center gap-3 py-4 bg-emerald-50 border-2 border-emerald-500 rounded-xl"
                      onPress={() => {
                        reorder(selectedOrder);
                        closeOrderDetails();
                      }}
                    >
                      ...
                      <Text className="text-emerald-700 font-medium">Reorder All</Text>
                    </TouchableOpacity>
                  )}
                  <TouchableOpacity
                    className="flex-1 flex-row items-center justify-center gap-3 py-4 bg-emerald-600 rounded-xl shadow-lg"
                    onPress={() => downloadInvoice(selectedOrder)}
                  >
                    <Download size={20} color="#fff" />
                    <Text className="text-white font-medium">Invoice</Text>
                  </TouchableOpacity>
                </View>
              </ScrollView>
            )}
      ...
      >
        <View className="flex-1 bg-black/50 justify-center items-center p-6">
          <View className="w-full bg-white rounded-2xl p-8">
            <View className="items-center mb-6">
              <View className="w-20 h-20 rounded-full bg-emerald-100 justify-center items-center mb-4">
                <AlertCircle size={40} color="#059669" />
              </View>
              <Text className="text-2xl font-medium text-gray-900 mb-2">Login Required</Text>
              <Text className="text-gray-600 text-center">
                You need to login to view your orders and track your purchases
              </Text>
            </View>
