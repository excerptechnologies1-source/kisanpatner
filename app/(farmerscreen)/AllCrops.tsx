...
const Mycrop: React.FC = () => {
  ...
  const getOfferStatusColor = (status: string) => {
    const colors = {
      ...
      countered: 'bg-blue-100 border-blue-300'
    };
 ...
 <ScrollView
        ...
        <View className="mx-5 mb-4 p-4 bg-red-50 rounded-xl border border-red-200">
          <View className="flex-row items-start">
            ...
            <View className="flex-1">
              <Text className="font-subheading text-red-800 mb-1">Unable to load products</Text>
      ...
      <View className="px-4 ">
        {products.map((product, index) => {
          ...
          return (
            <View 
              key={product._id} 
              className="bg-white rounded-lg mb-4 overflow-hidden border border-gray-200 px-1"
            >
              ...
              <View className="relative">
                ...
                <View className={`absolute top-3 left-3 ${statusBadge.bg} px-3 py-1.5 rounded-full`}>
                  <Text className={`text-xs font-subheading ${statusBadge.text}`}>
                    {statusBadge.label}
                  </Text>
                </View>
              ...
              <View className="p-4">
                {/* Title and Category */}
                <View className="mb-3">
                  <Text className="text-lg font-subheading text-gray-900 mb-1" numberOfLines={1}>
  ...
  <View className="mb-4">
    <View className="flex-row justify-between items-center mb-3 px-1">
      <Text className="text-lg font-subheading text-gray-900">Grade Prices</Text>
      <Text className="text-sm text-gray-500">
        {product.gradePrices.length} grades
      </Text>
    </View>
    ...
    >
      {product.gradePrices.map((grade, gradeIndex) => (
        <View 
          key={grade._id} 
          className="w-80 bg-white border border-gray-300 rounded-xl p-4 mr-3 shadow-sm"
          style={{ minHeight: 200 }}
        >
          {/* Grade Header */}
          <View className="flex-row justify-between items-start mb-3">
            <View className="flex-1">
              <Text className="font-subheading text-gray-900 text-lg" numberOfLines={1}>
                {grade.grade} Grade
              </Text>
              <View className="flex-row items-center mt-1">
                <View className="bg-green-100 px-2 py-1 rounded">
                  <Text className="text-xs font-medium text-green-800">
                    {grade.totalQty} {product.unitMeasurement || 'units'} available
                  </Text>
                </View>
            ...
            <View className="items-end ml-2">
              <Text className="text-2xl font-subheadingtext-green-600">
                ₹{grade.pricePerUnit}
              </Text>
          ...
          {grade.purchaseHistory && grade.purchaseHistory.length > 0 && (
            <View className="mb-3">
              <View className="flex-row items-center mb-2">
                ...
                <Text className="font-medium text-gray-900 text-sm ml-1.5">Recent Sales</Text>
                <Text className="text-xs text-gray-500 ml-auto">
                  {grade.purchaseHistory.length} sales
                </Text>
              </View>
              {grade.purchaseHistory.slice(0, 1).map((purchase, idx) => (
                <View 
                  key={idx} 
                  className="bg-green-50 p-2.5 rounded-lg border border-green-100"
                >
                  <View className="flex-row justify-between items-center">
                    ...
                    <View className="items-end ml-2">
                      <Text className="text-sm font-subheading text-gray-900">
          ...
          {/* Action Buttons - Stacked for Horizontal Layout */}
          <View className="mb-3">
            <TouchableOpacity
              className={`flex-row items-center justify-center py-2 rounded-lg mb-2 ${
                grade.totalQty === 0 
                  ? 'bg-gray-200' 
                  : 'bg-green-600'
              }`}
            ...
            >
              <Check size={14} color={grade.totalQty === 0 ? "#9ca3af" : "#ffffff"} />
          ...
          {grade.offers && grade.offers.length > 0 && (
            <View className="mt-2">
              <View className="flex-row items-center justify-between mb-2">
                <Text className="text-sm font-medium text-gray-900">Offers</Text>
                <View className="flex-row">
                  {(() => {
                    const pending = grade.offers.filter(o => o.status === 'pending').length;
                    const accepted = grade.offers.filter(o => o.status === 'accepted').length;
                    const countered = grade.offers.filter(o => o.status === 'countered').length;
                    
                    return (
                      <>
                        {pending > 0 && (
                          <View className="bg-yellow-100 px-1.5 py-0.5 rounded mr-1">
                            <Text className="text-[10px] font-medium text-yellow-800">
                              {pending} Pending
                            </Text>
                          </View>
                        )}
                        {accepted > 0 && (
                          <View className="bg-green-100 px-1.5 py-0.5 rounded mr-1">
              ...
              {/* Show first pending offer */}
              {grade.offers.filter(o => o.status === 'pending').slice(0, 1).map((offer) => (
                <View 
                  key={offer._id} 
                ...
                >
                  <View className="flex-row justify-between items-center">
                    <View className="flex-1">
                      <Text className="text-xs font-medium text-gray-900" numberOfLines={1}>
                        Trader {offer.traderId?.substring(0, 8)}...
                      </Text>
                    ...
                    >
                      <Text className="text-[10px] font-medium text-white">View</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ))}
              ...
              <View className="p-5">
                ...
                {directOfferPrice && directOfferQuantity && (
                  <View className="bg-green-50 p-4 rounded-xl border border-green-200">
                    ...
                    <Text className="text-2xl font-subheadingtext-green-700">
            ...
            <ScrollView className="max-h-96">
              <View className="p-5">
                <View className="bg-gray-50 p-4 rounded-xl mb-4">
                  ...
                  <Text className="text-lg font-subheadingtext-gray-900">
                ...
                {counterPrice && counterQuantity && (
                  <View className="bg-blue-50 p-4 rounded-xl border border-blue-200">
                    ...
                    <Text className="text-2xl font-subheadingtext-blue-700">
