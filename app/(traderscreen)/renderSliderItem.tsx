...
const renderSliderItem = ({ item, onPress }: { item: { id: string, img: string, title: string, des: string }, onPress?: () => void }) => {
  const router = useRouter()
  return (
    <TouchableOpacity
      activeOpacity={0.9}
      className="w-64 mr-4 rounded-md overflow-hidden bg-white shadow-lg"
      onPress={onPress || (() => console.log("Slider item pressed:", item.id))}
    >
      <Image
        source={{ uri: item.img }}
      ...
      >
        <Text className="text-white font-medium text-sm mb-1">{item.title}</Text>
        <Text className="text-white text-sm opacity-90">{item.des}</Text>
        <TouchableOpacity
          onPress={() => router.push("/(traderscreen)/Allcrops")}
          className="w-[95%] bg-green-400 mt-3 flex rounded justify-center items-center p-2"
        >
